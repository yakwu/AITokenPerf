...
class TestConnectivityProbe:
    @pytest.mark.asyncio
    async def test_passes_on_valid_response(self):
        ...
        async def iter_content():
            ...
            yield _sse("content_block_delta", delta={"type": "text_delta", "text": "hello"})
        ...
        result = await run_connectivity_probes(config, mock_session, "t", 30)
        assert result.status == "passed"
        assert len(result.probes) == 1

    @pytest.mark.asyncio
    async def test_fails_on_empty_output(self):
        from app.diagnostics.connectivity import run_connectivity_probes
        mock_resp = MagicMock()
        mock_resp.status = 200
        ...
        async def iter_content():
            yield _sse("message_start", message={"usage": {"input_tokens": 10}})
            yield _sse("content_block_delta", delta={"type": "text_delta", "text": ""})
            yield _sse("message_delta", usage={"output_tokens": 0}, delta={"stop_reason": "end_turn"})
            yield _sse("message_stop")
